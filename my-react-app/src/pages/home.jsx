
function HomePage(){

    return(
        <>
        <header>

            <div>
                <img src="logo-base1.jpg" alt="Logotipo Pagina CPCE"/>
                <nav>
                    <li><a href="#">Inicio</a></li>
                    <li><a href="#">Otro Link</a></li>
                    <li><a href="#">Otro Link</a></li>
                    <li><a href="#">Reservar</a></li>
                </nav>
            </div>
      
            <div> 
                <div>
                    <h1>Reserva tu cancha al instante</h1>
                    <button>Reservar</button>
                </div>
            <div>
          
          <img src="canchaPadel.png" alt="Imagen Padel"/>
        </div>
        </div>
    </header>
    
    <article> 
      <div> 
        <h2>Encuentra tu ritmo, mejora tu juego.</h2>
        <p>Bienvenidos, ofrecemos una cancha de alta calidad disponibles 
          para jugadores de todos los niveles. Ya sea para un partido amistoso
          o para entrenamientos serios, nuestras instalaciones estan diseñadas
          para proporcionar la mejor experiencia de juego.</p>
      </div>
      
      <div> 
        <div>
          <img src="" alt=""/>
          <p>Superficies de juego profesionales</p>
        </div>
        <div>
          <img src="" alt=""/>
          <p> Iluminacion optima para partidos nocturnos.</p>
        </div>
        <div>
          <img src="" alt=""/>
          <p>Vestuarios comodos y equipados.</p>
        </div>
      </div>

    </article>
    
    <article> 
      <div>
        <h1>Reservar a solo $2000 por hora</h1>
      </div>
      <div>
        <p>Reserva tu turno facilmente</p>
        <ol>
          <li>Completa el formulario con tus datos.</li>
          <li>Selecciona la fecha y hora deseada.</li>
          <li>Carga el comprobante de pago para confirmar tu turno.</li>
        </ol>
        <button>Reservar</button>
      </div>
    </article>
    
    <footer> 
      <div>
        <p>Contacto</p>
        <p>380-4533470 | 380-4797455</p>
      </div> 

      <div>
        <p>Direccion</p>
        <p>Castro Barros 1102, F5300 La Rioja</p>
      </div> 
      
    </footer>
    </>
    );
}

export default HomePage;